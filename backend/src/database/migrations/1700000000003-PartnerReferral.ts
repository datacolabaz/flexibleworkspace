...
/**
 * Partner / Affiliate / Referral System — embeds
 * docs/phase2/32_PARTNER_REFERRAL_DDL.sql VERBATIM, exactly as InitSchema and
 * SeedReferenceData embed their respective Phase 2 SQL files, to avoid any
 * transcription drift between the approved schema addendum and what actually
 * runs. Approved per ADR-010 (docs/27_ADRS.md) and
 * docs/31_PARTNER_REFERRAL_ARCHITECTURE.md before Phase 4 implementation
 * continued past the Auth module.
 *
 * This migration makes exactly one change to previously-approved Phase 2
 * tables (ledger_entry.provider_id / payout.provider_id become nullable,
 * each gains a nullable partner_id + an exclusive-OR CHECK constraint) —
 * everything else is new tables only. See §31.5 of the architecture doc for
 * the full rationale.
 *
 * Source: docs/phase2/32_PARTNER_REFERRAL_DDL.sql
 */
export class PartnerReferral1700000000003 implements MigrationInterface {
  name = 'PartnerReferral1700000000003';
...
`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
-- Reverse of 32_PARTNER_REFERRAL_DDL.sql, in dependency order.
DROP TRIGGER IF EXISTS trg_partner_updated_at ON partner;

ALTER TABLE payout DROP CONSTRAINT IF EXISTS chk_payout_payee_exclusive;
DROP INDEX IF EXISTS idx_payout_partner_status;
ALTER TABLE payout DROP COLUMN IF EXISTS partner_id;
ALTER TABLE payout ALTER COLUMN provider_id SET NOT NULL;

ALTER TABLE ledger_entry DROP CONSTRAINT IF EXISTS chk_ledger_entry_payee_exclusive;
DROP INDEX IF EXISTS idx_ledger_entry_partner_created;
ALTER TABLE ledger_entry DROP COLUMN IF EXISTS partner_id;
ALTER TABLE ledger_entry ALTER COLUMN provider_id SET NOT NULL;

DROP TABLE IF EXISTS booking_referral_attribution;
DROP TABLE IF EXISTS referral_click;
DROP TABLE IF EXISTS referral_campaign;
DROP TABLE IF EXISTS partner;

DROP TYPE IF EXISTS partner_commission_type;
DROP TYPE IF EXISTS referral_campaign_status;
DROP TYPE IF EXISTS partner_status;
DROP TYPE IF EXISTS partner_type;
-- Note: PostgreSQL cannot DROP VALUE from an enum type, so the additive
-- PARTNER_COMMISSION value on ledger_entry_type is NOT removed by this
-- down() migration. This is a documented, deliberate limitation (matches
-- Postgres capabilities, not an oversight) — reversing it fully would
-- require recreating ledger_entry_type end-to-end.
`);
  }
}
