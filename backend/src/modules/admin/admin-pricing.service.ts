...
export class AdminPricingService {
  constructor(
    @InjectRepository(AdminPricingSettingEntity)
    private readonly settingRepo: Repository<AdminPricingSettingEntity>,
    @InjectRepository(CommissionRuleEntity)
    private readonly commissionRepo: Repository<CommissionRuleEntity>,
  ...
  async updateDefault(adminUserId: string, dto: UpdatePricingSettingDto) {
    ...
    const before = setting
      ? {
          ...
          minimumPriceAmount: setting.minimumPriceAmount,
    ...
    let commissionRule = await this.commissionRepo.findOne({
      where: { scope: CommissionRuleScope.PLATFORM_DEFAULT },
    });
    commissionRule ??= this.commissionRepo.create({
      scope: CommissionRuleScope.PLATFORM_DEFAULT,
      priority: 0,
    });
    ...
    await this.auditLogService.recordChange({
      ...
      afterState: {
        ...
        minimumPriceAmount: saved.minimumPriceAmount,
