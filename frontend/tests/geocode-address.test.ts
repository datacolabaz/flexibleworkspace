...
describe('geocodeAddress', () => {
  ...
  it('throws on a Mapbox API failure so the UI can show a retry/manual-pin state', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(new Response(null, { status: 429 })));

    await expect(geocodeAddress('Mətbuat prospekti, Bakı, Azərbaycan', 'pk.test')).rejects.toThrow('Mapbox geocoding failed with status 429.');
  });
